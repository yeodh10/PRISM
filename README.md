...
> 개인정보·데이터 규제 법령(개인정보보호법·정보통신망법·신용정보법·위치정보법·AI기본법·통신비밀보호법)을 자연어로 질문하면, 관련 조문을 **검색**해 쉬운 말로 답하고 **출처(법령·조/항/호)를 인용**하는 RAG 챗봇.
...
LLM을 빼고 **검색 단계만** 측정한다(질문 임베딩 → 벡터 top-k). 정답은 "가장 관련 깊은 (법령, 조문)"으로 큐레이션한 **43개 질의(6개 법령) 기준**:

| 지표 | 값 |
|---|---|
| recall@1 | **0.791** (34/43) |
| recall@4 | **0.930** (40/43) |
| MRR | **0.843** |
...
> 점수를 100%로 맞추지 않았다 — 미스는 실제 한계다. 예) "동의 없이 수집"은 제15조 대신 인접 조문을, "불법 유출 벌칙"은 제71조(벌칙) 대신 제59조(금지행위·유출 금지)를 상위로 반환. **정직한 측정**이 목적.
...
### 2. 한국어 임베딩 선택
영어 모델은 한국어 법률 용어 검색 품질이 낮음. `ko-sroberta-multitask`(한국어 SBERT)를 **로컬 추론**으로 채택 → API 비용 0, 데이터 외부 미전송. L2 정규화 후 코사인 유사도로 검색. **품질 측정(43문항·6법령·(법령,조문) 정답 기준): recall@1 0.79 · recall@4 0.93 · MRR 0.84** — `python -m scripts.eval_retrieval`로 재현·회귀 감지(시행령 질의 rank-1 4/4; 모법↔시행령 주제 중첩 시 양방향 링크가 둘 다 표면화).

### 3. 법령 데이터 정확성 (날조 0)
법조문은 한 글자도 틀리면 안 됨. 내 기억이 아니라 **국가법령정보센터(law.go.kr) 원문**을 수집. 병렬 에이전트로 **6개 법령 + 개인정보보호법 시행령, 총 117개 핵심 조문**을 모으고, **독립 재페치로 교차검증**(서로 다른 출처가 호 개수·중간점까지 일치하는지) + 핵심 조문은 직접 스폿체크. 현행 버전 명시(개인정보 보호법 법률 제20897호, 시행령 대통령령 제36340호 등). 과징금은 추측성 "10%"가 아닌 **현행 3%**를 정확히 반영. 검증 불가한 조문은 날조 대신 제외 — 위치정보법은 law.go.kr 접근 오류로 1차 보류했다가 **CaseNote·bigcase.ai 교차검증으로 재수록**, 정보통신망법 제71조는 누락 호를 원문 확인 후 보강, 시행령 제39/40조(통지·신고 맞교체)는 현행 번호 확인.
...
- **법령·조문 범위**: **6개 법령 + 개인정보보호법 시행령, 총 117개 조문** — 개인정보보호법(수집·이용·동의·정보주체 권리·가명정보·적용범위·CPO·감독제재·손해배상·과징금 등 53), 정보통신망법(불법정보·침해금지·스팸·본인확인·정보보호책임자·침해사고 등 19), 신용정보법(개인신용정보·자동화평가·마이데이터·손해배상 등 13), 위치정보법(수집·이용·정보주체 권리·긴급구조·8세 이하 아동 보호 등 12), AI기본법(고영향 AI·생성형 AI 투명성·영향평가·윤리원칙 등 7), 통신비밀보호법(통신·대화 비밀·감청·통신사실확인자료·통신제한조치 등 6), 개인정보보호법 시행령(안전조치·영향평가·유출 통지/신고 등 7). law.go.kr로 추가 법령·조문 확장 가능. **법령 필터·크로스-법령 상호참조·법↔시행령 연결 ✅ 구현**(예: 신용정보법 제40조 → 정보통신망법 제50조 준용, 법 제29조 안전조치 → 시행령 제30조 세부기준). *구어체 쿼리 리랭킹·타 법 시행령은 향후 과제.*
- **시행령 핵심 연결 ✅**: 개인정보보호법 시행령 7조문(안전조치·영향평가·유출 통지/신고 등)을 법↔시행령 양방향 그래프로 연결 — 법 조문을 물으면 위임된 시행령 세부기준(72시간·암호화·5만명 기준 등)이 함께 표면화. 타 법 시행령·개인정보위 해설서는 향후 확장.
- **Hybrid RAG ✅ 구현**: 조문 간 상호참조를 그래프로 구조화(Vector+구조화)해 연관 조문을 함께 반환 — **법령 내부 + 크로스-법령(준용·근거)** 모두. 법↔**시행령** 연결은 시행령 데이터 확보 후 확장 예정.
...
├── eval/eval_set.json     # 검색 평가 질의셋(43 · 6개 법령 · law-aware)
├── DESIGN.md              # 설계 결정 기록(ADR)
├── data/                  # 법령별 조문 JSON(pipa·network_act·credit_info·ai_act·location_info·pipa_decree·comm_secret) 117개
