...
### 3. 법령 데이터 정확성 (날조 0)
법조문은 한 글자도 틀리면 안 됨. 내 기억이 아니라 **국가법령정보센터(law.go.kr)의 조문별 `print` URL 원문**을 수집. 병렬 에이전트로 37개 핵심 조문을 모으고, **독립 재페치로 교차검증**(제15조 호 개수, 제21조 항 개수 등). 현행 버전(법률 제20897호, 시행 2025-10-02) 명시. 과징금은 추측성 "10%"가 아닌 **현행 3%**를 정확히 반영.
...
- **조문 범위**: 핵심 37개 조문(수집·이용·동의·아동·제3자제공·위탁·국외이전·파기·정보주체 권리〈열람·정정·삭제·처리정지·전송요구·자동화결정 거부〉·CPO·영향평가·과징금·손해배상·금지행위·벌칙 등). law.go.kr OpenAPI로 전체 자동 수집 확장 가능.
