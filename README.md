...
### 2. 한국어 임베딩 선택
영어 모델은 한국어 법률 용어 검색 품질이 낮음. `ko-sroberta-multitask`(한국어 SBERT)를 **로컬 추론**으로 채택 → API 비용 0, 데이터 외부 미전송. L2 정규화 후 코사인 유사도로 검색. **품질 측정(39문항·5법령·(법령,조문) 정답 기준): recall@1 0.82 · recall@4 0.92 · MRR 0.86** — `python -m scripts.eval_retrieval`로 재현·회귀 감지.

### 3. 법령 데이터 정확성 (날조 0)
법조문은 한 글자도 틀리면 안 됨. 내 기억이 아니라 **국가법령정보센터(law.go.kr) 원문**을 수집. 병렬 에이전트로 **5개 법령(개인정보보호법·정보통신망법·신용정보법·위치정보법·AI기본법) 81개 핵심 조문**을 모으고, **독립 재페치로 교차검증**(서로 다른 출처가 호 개수·중간점까지 일치하는지). 현행 버전 명시(개인정보 보호법 법률 제20897호 등). 과징금은 추측성 "10%"가 아닌 **현행 3%**를 정확히 반영. 검증 불가한 조문은 날조 대신 제외 — 위치정보법은 law.go.kr 접근 오류로 1차 보류했다가 **CaseNote·bigcase.ai 교차검증으로 재수록**, 정보통신망법 제71조는 누락 호를 원문 확인 후 보강.
...
- **법령·조문 범위**: **5개 법령 81개 조문** — 개인정보보호법(수집·이용·동의·정보주체 권리·CPO·과징금 등 37), 정보통신망법(불법정보·침해금지·스팸·침해사고 신고·정보보호조치 등 12), 신용정보법(개인신용정보·자동화평가·마이데이터·손해배상 등 13), 위치정보법(수집·이용·정보주체 권리·긴급구조·8세 이하 아동 보호 등 12), AI기본법(고영향 AI·생성형 AI 투명성·영향평가·윤리원칙 등 7). law.go.kr로 추가 법령·조문 확장 가능. **법령 필터(전체/법령별)·크로스-법령 상호참조 ✅ 구현**(예: 신용정보법 제40조 → 정보통신망법 제50조 준용). *시행령 연결·구어체 쿼리 리랭킹은 향후 과제.*
...
├── eval/eval_set.json     # 검색 평가 질의셋(39 · 5개 법령 · law-aware)
├── DESIGN.md              # 설계 결정 기록(ADR)
├── data/                  # 법령별 조문 JSON(pipa·network_act·credit_info·ai_act·location_info) 81개
